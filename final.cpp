...
typedef struct job {
    ...
    int remainTime;//剩下运行时间
    int flag;//用来标志作业，0未进入内存，1进入内存
    int justTime;//用于在抢占式中，记录每一次开始运行的时间
    //int n;//在进入内存时，记录之前的位置
} JOB;
...
int recursionCompute(vector<JOB> *middle, vector<JOB> *result, vector<JOB> *process, tm arriveTime, tm endTime,
                     time_t remainTime);
...
void FCFS(vector<JOB> start, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i == 0) {
            ...
            int lastEndtime = result.at(i - 1).endTime;
...
//                }
//                justRun.remainTime =
//                        (justRun.remainTime * 60 - (mktime(&temp.arriveTime) - mktime(&justRun.justTime))) / 60;
...
void takeJOB(JOB *a) {
    ...
    //计算周转时间
    a->minute = a->endTime-a->arriveTime;
    a->flag = 1;
}
...
void replaceJOB(vector<JOB> *ve, JOB temp, int i) {
    ...
    ve->erase(ve->begin() + i);
    //向指定位置i处增加一个元素
    ve->insert((ve->begin() + i), temp);
}

void displayJOB(JOB a) {

    cout << "作业名：" << a.name << endl;
    cout << "到达时间为:" << a.arriveTime << endl;
    cout << "估计运行时间为:" << a.runTime << endl;
    cout << "进入内存时刻为:" << a.inTime << endl;
    cout << "进入运行结束时刻为:" << a.endTime << endl;
    cout << "作业周转时间为:" << a.minute << endl;
